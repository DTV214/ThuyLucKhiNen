...
import { Menu, PhoneCall, Search, Send, ShieldCheck } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 border-b border-outline-variant/30 bg-surface/90 shadow-[0_1px_8px_rgba(0,0,0,0.04)] backdrop-blur-xl">
      ...
      <div className="border-b border-outline-variant/20 bg-surface-container-low px-gutter py-1.5">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 text-body-sm text-on-surface-variant">
          <a href={companyInfo.primaryPhoneHref} className="inline-flex items-center gap-1.5 transition-colors hover:text-primary">
            <PhoneCall aria-hidden="true" className="size-4 text-primary" />
            <span>Hotline: {companyInfo.phoneDisplay}</span>
          </a>
      ...
      <div className="mx-auto flex h-20 max-w-7xl items-center gap-6 px-gutter">
        ...
        <form role="search" className="relative hidden min-w-0 max-w-md flex-1 lg:block">
          ...
          <input id="site-search" name="q" type="search" placeholder="Tìm mã phụ tùng, model thiết bị..." className="h-10 w-full rounded-xl border border-outline-variant/40 bg-surface-container py-2 pl-10 pr-4 text-body-md outline-none transition-colors placeholder:text-outline focus:border-primary" />
        </form>
        ...
        <Link href="/yeu-cau-bao-gia" className="hidden items-center gap-2 rounded-xl bg-primary-container px-4 py-2.5 text-label-md font-semibold text-on-primary-container shadow-sm transition-colors hover:bg-primary sm:inline-flex">
          <Send aria-hidden="true" className="size-4" /> Yêu cầu báo giá
        </Link>
